...
- `docs/apple-silicon-mac-adaptation-plan.md`

## 当前分支定位

- 当前分支是 `Apple Silicon macOS` 适配分支。
- 目标平台只包括 `M1 / M2 / M3 / M4` 及之后的 `arm64 macOS`。
- 当前分支不再兼容 Windows，也不覆盖 Intel Mac。
- 仓库里仍可能残留 Windows 实现、`.exe` 命名、`%LOCALAPPDATA%` 路径、`taskkill`、`uiautomation` 等旧事实；在这个分支上修改时，以“迁到 mac-only”为准，不再为 Windows 回归负责。
...
- `wechat_auto/window.py`：微信主窗口定位与过滤逻辑；当前仍有 Windows 遗留实现，mac 适配时优先改这里。
- `wechat_auto/controls.py`：控件树定位与文本判定工具；当前仍偏 Windows/UIA 实现，mac 适配时优先改这里。
...
- `listener_app/group_listener_worker.py`：监听 worker；输出 JSON 行事件。当前读取链路仍偏 Windows/UIA，是本分支重点迁移对象。
...
- `listener_app/sidebar_tts.py`：TTS runtime、依赖探测与播放器工厂；当前默认链路仍偏 Windows，是本分支重点迁移对象。
...
### 脚本与文档
- `scripts/build_desktop_shell_sidecars.py`：sidecar 构建脚本；当前仍按 Windows `.exe` 路径组织，mac 版需要重写。
...
- `docs/apple-silicon-mac-adaptation-plan.md`：本分支的 mac-only 迁移范围、阶段计划与验证口径。
- `logs/`：源码态运行日志目录。
...
## 维护约束
- 当前分支的根目标是把仓库收敛到 `Apple Silicon macOS` 主路径；不要再新增任何 Windows 兼容分支逻辑。
