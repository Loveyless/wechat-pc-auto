...
#[tauri::command]
fn get_backend_connection_info(state: State<'_, ManagedBackendState>) -> BackendConnectionInfo {
    state.info.clone()
}
...
fn managed_runtime_root<R: Runtime>(app: &tauri::App<R>) -> Result<PathBuf, String> {
    let runtime_root = app
        .path()
        .app_local_data_dir()
        .map_err(|err| format!("resolve app_local_data_dir failed: {err}"))?;
...
fn acquire_backend_bootstrap_lock(runtime_root: &Path) -> Result<BackendBootstrapLock, String> {
    let name = build_backend_bootstrap_mutex_name(runtime_root);
    ...
    let wait_status = unsafe { WaitForSingleObject(handle, BACKEND_BOOTSTRAP_LOCK_TIMEOUT_MS) };
    match wait_status {
        ...
        WAIT_TIMEOUT => {
            ...
            Err(format!(
                "wait backend bootstrap lock timeout after {}ms",
                BACKEND_BOOTSTRAP_LOCK_TIMEOUT_MS
            ))
        }
        WAIT_FAILED => {
...
fn spawn_backend_sidecar<R: Runtime>(
    ...
    let log_root = runtime_root.clone();
...
fn bootstrap_backend<R: Runtime>(app: &tauri::App<R>) -> ManagedBackendState {
    let runtime_root = match managed_runtime_root(app) {
        Ok(path) => path,
        Err(error) => {
    ...
    let result = match spawn_backend_sidecar(app, &runtime_root) {
