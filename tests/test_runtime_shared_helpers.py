...
class RuntimeSharedHelpersTest(unittest.TestCase):
    ...
    def test_build_less_tts_curl_command_uses_skill_defaults(self):
        ...
        command = sidebar.build_less_tts_curl_command(
            settings,
            "你好",
            headers_output_path="D:\\tmp\\headers.txt",
            body_output_path="D:\\tmp\\body.mp3",
        )
    ...
    def test_create_tts_player_less_tts_reports_missing_curl(self):
        ...
        with mock.patch.object(sidebar, "load_less_tts_settings", return_value=settings), mock.patch.object(
            ...
            sidebar,
            "probe_less_tts_curl_runtime",
            return_value="missing executable 'curl.exe': not found",
        ):
            player, runtime_text = sidebar.create_tts_player(
                {
                    "provider": "less_tts",
        ...
        self.assertIn("missing executable 'curl.exe'", runtime_text)
    ...
    def test_check_tts_dependency_packaging_accepts_less_tts_curl_runtime(self):
        with mock.patch.object(
            sidebar,
            "probe_less_tts_curl_runtime",
            return_value="",
        ...
        self.assertTrue(ok)
        self.assertIn("tts dependency check passed backend=less_tts module=curl.exe", detail)
    ...
    def test_check_tts_dependency_packaging_reports_missing_less_tts_curl(self):
        ...
        self.assertIn("missing executable 'curl.exe'", detail)
