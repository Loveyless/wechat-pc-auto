...
### 22.1) 设置页配置测试不能污染正式运行态
现象：
- 用户在设置页里改完 `deeplx` / `openai_compatible` / TTS provider 后，希望立刻点“测试”验证当前草稿配置。
- 如果测试直接复用正式运行态对象或顺手写回 `listener.json`，就会把“只是想试一下”变成真实配置变更，甚至影响当前自动翻译/自动朗读链路。

处理：
- 设置页的“测试翻译”与“测试朗读”必须走隔离临时配置：先把当前草稿 payload 合并到临时 runtime root，再按临时配置实例化 translator / TTS player。
- 翻译测试只吸收 `translate` 和 `secret_updates.translate` 草稿；朗读测试只吸收 `tts` 和 `secret_updates.tts` 草稿。另一侧未保存、未填完的 provider 配置不能拦住当前测试。
- 测试接口不写回真实 `listener.json`，也不修改当前 runtime 的 `translate_enabled`、`tts.auto_read_enabled` 或会话状态。
- 翻译测试固定使用中文样例文本，直接回传译文或失败原因；这样用户不用先等真实微信消息进来。
- 朗读测试固定使用英文样例文本，并走阻塞式一次性试播；不能只返回“已入队”，否则 provider 真正合成/播放失败时设置页会误报成功。
- 由于当前 TTS 链路只接受可朗读英文，朗读测试文本也必须保持英文，不能拿中文样例去测云 TTS。
