...
- [ ] 1.2 Update `listener_app/runtime_config_store.py` to expose/save provider-aware translate DTOs and per-provider TTS `config_path` maps with write-only secret handling and unchanged file-boundary persistence
...
- [ ] 2.2 Reshape the settings workspace so translate renders `deeplx` / `openai_compatible` / `passthrough` branches and secondary TTS tuning moves behind advanced sections
- [ ] 2.3 Add frontend regression tests for translate provider branching, secret input modes, advanced TTS sections, and save payload generation
