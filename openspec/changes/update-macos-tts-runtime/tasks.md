...
- [ ] 1.2 Update `listener_app/runtime_config.py`, `listener_app/runtime_config_store.py`, and `listener_app/runtime_api.py` so the authoritative TTS provider enum, defaults, config snapshots, and save behavior expose `macos_system` while preserving existing file-boundary persistence and legacy-read compatibility
